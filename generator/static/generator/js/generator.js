...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const trackData = JSON.parse(
    document.getElementById("track-data").textContent,
  );
...
function getTimes() {
  ...
  if (isNaN(startToSeconds) || isNaN(endToSeconds))
    throw new Error("Invalid time values");
  if (startToSeconds < 0 || endToSeconds < 0) throw new Error("Negative time");
  if (
    videoDuration &&
    (startToSeconds >= videoDuration || endToSeconds > videoDuration)
  ) {
    throw new Error("Out of video scope");
  }
  if (startToSeconds >= endToSeconds)
...
function getNewTimes() {
  ...
  const endToSeconds =
    startToSeconds + 5 > video.duration
  ...
  if (startToSeconds >= endToSeconds)
    throw new Error("Start must be before end");
  const formatedStart = `${Math.floor(startToSeconds / 3600)
    .toString()
    .padStart(2, "0")}:${Math.floor((startToSeconds % 3600) / 60)
    .toString()
    .padStart(2, "0")}:${Math.floor(startToSeconds % 60)
    .toString()
    .padStart(2, "0")}`;
  const formatedEnd = `${Math.floor(endToSeconds / 3600)
    .toString()
    .padStart(2, "0")}:${Math.floor((endToSeconds % 3600) / 60)
    .toString()
    .padStart(2, "0")}:${Math.floor(endToSeconds % 60)
    .toString()
    .padStart(2, "0")}`;
  return [formatedStart, formatedEnd, startToSeconds, endToSeconds];
}
...
function createCard(id, start, end, text) {
  ...
  newCard.querySelector(".cue-text-preview").textContent =
    text.length > 30 ? text.slice(0, 30) + "..." : text;
  return newCard;
}

function sortCuesRecord() {
  cuesRecord.sort((a, b) => a.startSeconds - b.startSeconds || a.id - b.id);
}
...
async function deleteCue(pk) {
  ...
  if (response.status == 200) {
    if (selectedCueId == pk) {
      ...
      selectedCueId =
        cuesRecord[deleteCueIndex - 1]?.id ||
        cuesRecord[deleteCueIndex + 1]?.id ||
        null;
    }
